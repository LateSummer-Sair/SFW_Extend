...
public class NapCatSkillLib {
    ...
    public static void initSkills(SkillBank bank) {
        ...
        // === 消息拆分 ===
        bank.addBuiltinSkill("消息拆分发送", "napcat/message", "回复超100字会自动拆分为多条；也可用 <split> 主动指定分割点",
            "QQ 回复超过 100 字会被自动合理切割后逐条发送，无需你干预，发送条数不设上限。\n\n" +
            "## 主动拆分（可选）\n" +
            "如需精确控制分割位置，可在回复文本中用 <split> 分隔符：\n" +
            "第一条消息<split>第二条消息<split>第三条消息\n\n" +
            "## 规则\n" +
            "- 每条消息尽量精炼，控制在 100 字以内\n" +
            "- <split> 是回复文本中的分隔符，不是工具\n" +
            "- 超长回复会自动切割，无需手动 <split>，条数不设上限",
            "builtin", "execq");
