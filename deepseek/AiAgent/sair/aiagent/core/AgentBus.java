...
public class AgentBus {
    ...
    /** 子 Agent 结果回传前的最大字符数（长折叠的分段结论需要足够空间，故放宽到 3000）。 */
    ...
    /** 超过该长度的任务不做结果缓存（防止 base64 图片等超长任务把内存撑爆）。 */
    private static final int RESULT_CACHE_KEY_MAX_CHARS = 2000;
    /** 结果缓存有效期（毫秒）。 */
    ...
    public static final class AgentDef {
        ...
        /**
         * 是否为「视觉兜底 Agent」：只有当当前通道模型不具备原生视觉时才可用/有意义。
         * <p>该 Agent 的执行模型固定为具备视觉的模型（{@code AiConfig.VISION_CAPABLE_MODEL}），
         * 且其自身不参与图片预处理（避免递归识图）。</p>
         */
        public final boolean visionFallback;
        /**
         * 是否整体跳过调用方的预处理（图片 + 折叠消息）。
         * <p>仅视觉兜底 Agent 使用：它本身就是识图执行者，若再参与预处理会递归。</p>
         */
        public final boolean skipPreprocess;
        /**
         * 是否仅跳过「折叠消息委派」而保留图片预处理。
         * <p>段落 Agent 使用：它的任务内容由父 Agent 写进 task 文本，
         * 若再自行检出同一份折叠消息就会无限递归；但它必须保留图片预处理，
         * 这样在有原生视觉的通道上它能自己看图，而不是被迫调用 vision 工具多跑一次视觉模型。</p>
         */
        public final boolean skipForwardDelegation;

        public AgentDef(String name, String systemPrompt, String model,
                        Function<ToolContext, List<ToolDefinition>> toolsProvider) {
            this(name, systemPrompt, model, toolsProvider, false, false, false);
        }

        public AgentDef(String name, String systemPrompt, String model,
                        Function<ToolContext, List<ToolDefinition>> toolsProvider,
                        boolean visionFallback) {
            this(name, systemPrompt, model, toolsProvider, visionFallback, visionFallback, visionFallback);
        }

        public AgentDef(String name, String systemPrompt, String model,
                        Function<ToolContext, List<ToolDefinition>> toolsProvider,
                        boolean visionFallback, boolean skipPreprocess, boolean skipForwardDelegation) {
            this.name = name;
            this.systemPrompt = systemPrompt;
            this.model = model;
            this.toolsProvider = toolsProvider;
            this.visionFallback = visionFallback;
            this.skipPreprocess = skipPreprocess;
            this.skipForwardDelegation = skipForwardDelegation;
        }
    }
    ...
    public String invoke(String name, String task, ToolContext ctx) {
        ...
        if (def == null) {
            return "[AgentBus] 未知 Agent: " + name + "（可用：" + availableAgentList(ctx) + "）";
        }
        // === 视觉能力门控 ===
        // 当前通道模型自己就能看图（如 Flash）时，视觉兜底 Agent 不启用：
        // 直接让主/段落 Agent 自己看原图，避免多余的第二次模型调用。
        if (def.visionFallback && ctx != null && ctx.hasNativeVision()) {
            AiAgentActivity.debugLog("[AgentBus] 视觉 Agent 未启用：当前模型 " + ctx.model + " 已具备原生视觉");
            return "[AgentBus] 当前模型（" + ctx.model + "）已原生支持视觉，可直接查看图片内容，"
                 + "无需调用 vision Agent。请直接结合你看图的结果作答。";
        }
        ...
        // 结果缓存：key 为「作用域|Agent名|任务全文」。任务文本过长（如含 base64 图片的识图任务）不缓存，
        // 避免把兆级字符串长期驻留内存（视觉结果另有 ImageRecognizer 专用 LRU 缓存）。
        // 作用域（群/私聊）必须进 key：子 Agent 的执行依赖 ctx 里的群上下文，
        // 只按「Agent名|任务」缓存会把 A 群的答案直接返回给 B 群（跨群串味 + 泄露）。
        final String scope = (ctx != null) ? ctx.scopeKey() : "?";
        final boolean cacheable = task != null && task.length() <= RESULT_CACHE_KEY_MAX_CHARS;
        final String cacheKey = scope + "|" + def.name + "|" + (task == null ? "" : task);
        if (cacheable) {
        ...
        try {
            ...
            // 预处理开关按 Agent 职责细分：
            //  · vision 兜底 Agent：整体关闭（它自己就是识图执行者）
            //  · passage 段落 Agent：仅关闭折叠消息委派（防递归），保留图片预处理（有原生视觉时自己看图）
            if (def.skipPreprocess) bridge.setPreprocessEnabled(false);
            if (def.skipForwardDelegation) bridge.setForwardDelegationEnabled(false);
     ...
     * 解析子 Agent 实际应使用的模型。
     * <ul>
     *   <li><b>视觉兜底 Agent</b>：固定使用具备视觉能力的模型（{@code AiConfig.VISION_CAPABLE_MODEL}），
     *       否则 Pro 通道下它自己也看不了图。</li>
     *   <li><b>main / passage</b>：跟随当前通道的模型（{@code ctx.model}），
     *       保持与主流程一致的上下文风格与视觉能力——Flash 通道下它们自己就能看图。</li>
     * </ul>
     */
    private static String resolveAgentModel(AgentDef def, ToolContext ctx) {
        if (def.visionFallback) {
            return AiConfig.getInstance().getVisionModel();
        }
        if (ctx != null && ctx.model != null && !ctx.model.trim().isEmpty()) {
            return ctx.model;
        }
        return def.model;
    }
    ...
    private String invokeWithTimeout(final FunctionCallingBridge bridge, final String task,
        ...
        final String agentModel = resolveAgentModel(def, ctx);
        Future<String> f = pool.submit(new Callable<String>() {
            @Override
            public String call() throws Exception {
                return bridge.runWithDispatcher(task, def.systemPrompt, null, merged, agentModel, dispatcher, ctx, null);
            }
        });
        try {
    ...
    /**
     * 当前上下文下可用的 Agent 清单。
     * <p>当前模型已具备原生视觉时不列出 vision（该 Agent 不启用）。</p>
     */
    public String availableAgentList(ToolContext ctx) {
        boolean nativeVision = (ctx != null && ctx.hasNativeVision());
        return nativeVision ? "main/passage" : "main/vision/passage";
    }
    ...
    public static List<ToolDefinition> callAgentTools() {
        ...
        tools.add(new ToolDefinition("call_agent",
                "唤起子 Agent 执行任务并返回结果。可用：main(主模型)/passage(折叠消息与段落分析)；"
              + "vision(看图兜底，仅当当前模型不具备视觉能力时才需要，Flash 模型可直接看图无需调用)")
                .addString("agent", "main/passage/vision")
                .addString("task", "子任务描述"));
        tools.add(new ToolDefinition("ask_agent",
                "向子 Agent 请教问题。可用：main/passage；vision(看图兜底，仅当当前模型不具备视觉能力时需要)")
                .addString("agent", "main/passage/vision")
