...
public class PromptManager {
    ...
    private static String buildDefaultSystemPrompt() {
        return "你是运行在SairFrameWork(SFW)中的AiAgent智能助手(控制台交互)。\n"
            ...
            + "①短期记忆上下文(已注入)→②长期记忆上下文(已注入)→③知识库笔记(已注入)→④纠错记录(已注入)→⑤联网搜索\n"
    ...
    private static String buildDefaultExecqPrompt() {
        return "你在SairFrameWork中通过QQ聊天。遵守以下规则:\n\n"
            ...
            + "## 图片识别\n"
            + "本系统已接入在线OCR，图片会自动预处理成「二维码+OCR文字」备注附在任务描述中。\n"
            + "- 当前消息含图片时，直接依据备注回答图片内容，不要说自己看不到图片或没有OCR能力\n"
            + "- 用户问「我刚发的图/上面那张图」但本条消息没有图片备注时，说明用户未引用或未附带图片，应引导用户「引用那张图片」或「重新发送」，而不是说没接OCR\n\n"
            + "## 操作标签\n"
